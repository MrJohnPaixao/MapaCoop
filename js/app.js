...
(async function App() {
  ...
  try {
    ...
    loadingEl.innerHTML = `
      <div style="text-align:center;padding:20px;color:#2d6a4f">
        <div style="font-size:2.5rem;margin-bottom:12px">⚠️</div>
        <p style="font-family:'Playfair Display',serif;margin-bottom:6px">Erro ao carregar dados</p>
        <small style="color:#6b7280">${err.message}</small><br>
        <button onclick="location.reload()" style="margin-top:14px;padding:8px 20px;background:#2d6a4f;color:white;border-radius:20px;border:none;cursor:pointer;font-size:0.85rem">
          Tentar novamente
        </button>
      </div>`;
    return;
